...
export function OrderingButtons({ onSelectOrderBy }) {
    const initState = [
        {
            id: 0,
            classname: "btn btn-info btn-sm rounded-sm m-2",
            name: "date_desc",
        ...
        {
            id: 1,
            classname: "btn btn-outline-info btn-sm rounded-sm m-2",
            name: "date_asc",
            ordering: "ordering=created_date",
            text: "قدیمی ترین"
        },
    ...
    const handleClick = (button) => {
        ...
        for (i; i < buttonsState.length; i++) {
            if (i === x) {
                buttonsStateCopy[i].classname = "btn btn-info btn-sm rounded-sm m-2 shadow-none"
            } else {
                buttonsStateCopy[i].classname = "btn btn-outline-info btn-sm rounded-sm m-2 shadow-none"
            }
        }
